// copied from https://github.com/hpoul/dart_logging_appenders/blob/main/lib/src/logrecord_formatter.dart
// copyright lies with the original author

import 'dart:convert';
import 'dart:isolate';

import 'package:logging/logging.dart';
import 'package:school_data_hub_server/src/utils/logger/ansi_code.dart' as ansi;
import 'package:school_data_hub_server/src/utils/logger/exception_chain.dart';
...
class ColorFormatter extends LogRecordFormatter {
  ...
  _AnsiCombination? _colorForLevel(Level level) {
    ...
    if (level <= Level.WARNING) {
      return _AnsiCombination.combine([ansi.orange, ansi.styleBold]);
    }
    if (level <= Level.SEVERE) {
      return _AnsiCombination.combine([ansi.red, ansi.styleBold]);
    }

    return _AnsiCombination.combine([ansi.lightMagenta, ansi.styleBold]);
  }
}
