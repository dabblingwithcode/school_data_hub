...
List<Widget> pupilCategoryStatusesList(PupilProxy pupil, BuildContext context) {
  if (pupil.supportCategoryStatuses != null) {
    ...
    for (SupportCategoryStatus status in pupil.supportCategoryStatuses!) {
      if (pupil.supportCategoryStatuses!.any(
        (element) =>
        ...
        if (!statusesWithDuplicateGoalCategory.containsKey(
          ...
          statusesWithDuplicateGoalCategory[(status.supportCategoryId)]!.add(
            status,
          );
        } else {
          statusesWithDuplicateGoalCategory[(status.supportCategoryId)]!.add(
        ...
        log(
          'Adding status vom ${status.createdAt.formatDateForUser()} erstellt von ${status.createdBy}',
        );
      } else {
        statusesWidgetList.add(
          Padding(
            ...
            child: SupportCategoryStatusCard(
              pupil: pupil,
              statusesWithSameGoalCategory: [status],
            ),
          ),
        );
      }
