...
class LessonCell extends WatchingWidget {
      ...
      return LongPressDraggable<ScheduledLesson>(
        ...
        child: DragTarget<ScheduledLesson>(
          onWillAcceptWithDetails: (details) {
            // Accept if it's a different lesson in the same time slot
            return details.data.id != lesson!.id &&
                details.data.scheduledAtId == lesson!.scheduledAtId;
          },
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 2),
          // Show teachers
          _buildTeacherInfo(context, userManager),
        ],
      ),
    );
  }
  ...
  Widget _buildTeacherInfo(BuildContext context, UserManager userManager) {
...
void _swapLessonOrder(
  ...
  // Swap the timetableSlotOrder values between the two lessons
  final draggedOrder = draggedLesson.timetableSlotOrder;
  final targetOrder = targetLesson.timetableSlotOrder;

  final updatedDraggedLesson = draggedLesson.copyWith(
    timetableSlotOrder: targetOrder,
  );
  final updatedTargetLesson = targetLesson.copyWith(
    timetableSlotOrder: draggedOrder,
  );

  timetableManager.updateScheduledLesson(updatedDraggedLesson);
  timetableManager.updateScheduledLesson(updatedTargetLesson);
...
void _moveLessonToSlot(
  ...
  // Move lesson to different time slot
  // Find the next available order for the target slot
  final nextAvailableOrder = timetableManager.getNextAvailableOrderForSlot(
    targetSlot.id!,
  );

  final updatedLesson = draggedLesson.copyWith(
    scheduledAtId: targetSlot.id!,
    scheduledAt: targetSlot,
    timetableSlotOrder: nextAvailableOrder,
  );

  timetableManager.updateScheduledLesson(updatedLesson);
