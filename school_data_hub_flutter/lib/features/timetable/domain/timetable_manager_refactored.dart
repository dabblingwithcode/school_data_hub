...
class TimetableManager extends ChangeNotifier {
  ...
  Future<void> addScheduledLesson(ScheduledLesson lesson) async {
    ...
    // Update local state - we need to refresh data since we can't directly modify ValueListenable
    await refreshData();
  }
  ...
  Future<void> addClassroom(Classroom classroom) async {
    await _crudManager.addClassroom(classroom);
    await refreshData();
  }

  Future<void> updateClassroom(Classroom classroom) async {
    await _crudManager.updateClassroom(classroom);
    await refreshData();
  }
  ...
  Future<void> addLessonGroup(LessonGroup lessonGroup) async {
    await _crudManager.addLessonGroup(lessonGroup);
    await refreshData();
  }

  Future<void> updateLessonGroup(LessonGroup lessonGroup) async {
    await _crudManager.updateLessonGroup(lessonGroup);
    await refreshData();
  }
  ...
  void addPupilToLessonGroup(int lessonGroupId, int pupilDataId) {
    // For now, we'll need to refresh data since we can't directly modify ValueListenable
    // In a future iteration, we could expose the underlying ValueNotifier
    refreshData();
  }
