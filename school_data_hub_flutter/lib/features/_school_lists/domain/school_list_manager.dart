...
import 'package:school_data_hub_flutter/features/_school_lists/data/school_list_api_service.dart';
...
class SchoolListManager with ChangeNotifier {
  ...
  void clearData() {
    ...
    _schoolLists.clear();
    for (final proxyMap in _schoolListIdPupilEntriesMap.values) {
      proxyMap.dispose();
    }
  ...
  void _updateCollectionsFromSchoolList(
    ...
    final existingIndex = _schoolLists.value.indexWhere((l) => l.id == listId);
    if (existingIndex != -1) {
  ...
  void deleteFromStream(int id) {
    ...
    if (_schoolListMap.containsKey(id)) {
      ...
      _schoolListIdPupilEntriesMap.remove(id)?.dispose();
  ...
  Future<void> deleteSchoolList(int listId) async {
    ...
    _schoolListIdPupilEntriesMap.remove(listId)?.dispose();
