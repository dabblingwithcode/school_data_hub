...
class MatrixBulkCredentialsService {
  ...
  /// Generates a bulk credentials PDF from an existing list of user+password
  /// entries. Returns null if [userCredentials] is empty.
  static Future<File?> generateBulkCredentialsPdf({
    required String matrixDomain,
    required List<Map<String, dynamic>> userCredentials,
    required bool isStaff,
  }) async {
    if (userCredentials.isEmpty) return null;
    return _generateBulkCredentialsPdf(
      matrixDomain: matrixDomain,
      userCredentials: userCredentials,
      isStaff: isStaff,
    );
  ...
  static pw.Widget _buildCredentialCard({
    ...
    final String matrixId = user.id!.replaceAll("@", "").split(":").first;

    String qrPassword = password.substring(0, password.length - 4);

    String pin = password.substring(password.length - 4);

    final String cleanDomain = domain.replaceAll("https://", "");
    final String qrCodeData = "$matrixId:$cleanDomain*$qrPassword";
    ...
    final qrCredentials = '$cleanDomain*$encryptedQrCodeData';
