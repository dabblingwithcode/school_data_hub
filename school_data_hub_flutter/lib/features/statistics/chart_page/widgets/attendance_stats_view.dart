...
class AttendanceStatsView extends StatefulWidget {
  ...
  @override
  State<AttendanceStatsView> createState() => _AttendanceStatsViewState();
}

class _AttendanceStatsViewState extends State<AttendanceStatsView> {
  ...
  void _onSelectionChanged(charts.SelectionModel<String> model) {
    ...
    if (selectedDatum.isNotEmpty) {
      ...
      final dateData = widget.attendanceChartData[chartData.date];
  ...
  List<charts.Series<ChartData, String>> _createAttendanceSeries() {
    ...
    if (!_hiddenSeries.contains('excused')) {
      final excusedData = widget.sortedSchooldays.map((schoolday) {
        final data = widget.attendanceChartData[schoolday.schoolday];
    ...
    if (!_hiddenSeries.contains('unexcused')) {
      final unexcusedData = widget.sortedSchooldays.map((schoolday) {
        final data = widget.attendanceChartData[schoolday.schoolday];
        final dateStr = _formatDateForChart(schoolday.schoolday);
        return ChartData(
          date: schoolday.schoolday,
    ...
    if (!_hiddenSeries.contains('goneHome')) {
      final goneHomeData = widget.sortedSchooldays.map((schoolday) {
        final data = widget.attendanceChartData[schoolday.schoolday];
        final dateStr = _formatDateForChart(schoolday.schoolday);
        return ChartData(
          date: schoolday.schoolday,
