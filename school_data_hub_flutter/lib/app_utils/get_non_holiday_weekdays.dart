...
Future<List<DateTime>> getNonHolidayWeekdays({
  ...
  final response = await http.get(
    Uri.parse('https://ferien-api.de/api/v1/holidays/NW'),
  );
  if (response.statusCode != 200) throw Exception('Failed to load holidays');

  final List<dynamic> holidaysJson = jsonDecode(response.body) as List<dynamic>;

  // Collect all holiday dates in the range
  final Set<DateTime> holidayDates = {};
  for (final holiday in holidaysJson) {
    final DateTime holidayStart = DateTime.parse(holiday['start'] as String);
    final DateTime holidayEnd = DateTime.parse(holiday['end'] as String);
    for (
      var d = holidayStart;
